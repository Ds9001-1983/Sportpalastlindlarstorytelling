...
// route = interne Unterseite (kein neuer Tab).
const ITEMS: { key: string; icon: LucideIcon; span: string; route?: string }[] = [
  { key: 'egym', icon: Dumbbell, span: 'md:col-span-3' },
  { key: 'kurse', icon: Users, span: 'md:col-span-3', route: SITE.routes.kurse },
  ...
  { key: 'physio', icon: Stethoscope, span: 'md:col-span-2', route: SITE.routes.physio },
];
    ...
    >
      <div className="mx-auto max-w-7xl px-6 py-24 md:px-10 md:py-32">
        ...
        <div className="mt-14 grid grid-cols-1 gap-4 md:grid-cols-6">
          {ITEMS.map(({ key, icon: Icon, span, route }) => {
            const inner = (
              <>
            ...
            return route ? (
              <Link key={key} href={route} data-reveal className={cardClass}>
                {inner}
              </Link>
            ) : (
